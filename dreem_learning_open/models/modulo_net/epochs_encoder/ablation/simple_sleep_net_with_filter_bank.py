...
from ..legacy.seq_sleep_net import FilterBankLayer

class SimpleSpectrogramEncoderWithFilterBank(nn.Module):
    ...
    def __init__(self, input_dim=129, filter_dim=30, n_channels=1, hidden_layers=64, bidir=True,
        ...
        self.filter_reduction = FilterBankLayer(input_dim, filter_dim, n_channels)
...
class SimpleSleepNetWithFilterBank(EpochEncoder):
    ...
    def __init__(self, group, net_parameters):
        ...
        """
        super(SimpleSleepNetWithFilterBank, self).__init__(group, net_parameters)

    def init_encoder(self):
        n_channels, n_freq_filters, seq_length = tuple(self.group['encoder_input_shape'][2:])
        self.encoder = SimpleSpectrogramEncoderWithFilterBank(input_dim=n_freq_filters,
