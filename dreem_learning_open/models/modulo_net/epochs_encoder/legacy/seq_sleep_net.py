...
class FilterBankLayer(nn.Module):
    ...
    def __init__(self, input_dim=129, filter_dim=20, n_channels=1):
        ...
        w = lin_tri_filter_shape(filter_dim, (input_dim - 1) * 2, 100, 0, 50)
        ...
        self.register_parameter('triangular_filter_shape', self.triangular_filter_bank)
    ...
    def forward(self, x):
        ...
        x = x.view(n_channels, input_filter_dim, batch_size * length)
        filter_weights = self.sigmoid(self.filter_weights)
        filter_weights = filter_weights * self.triangular_filter_bank
        x = filter_weights.transpose(1, 2).bmm(x)
        x = x.view(n_channels, self.filter_dim, batch_size, length).permute(2, 0, 1, 3)
        return x
...
class SpectrogramEncoder(nn.Module):
    ...
    def __init__(self, input_dim=129, filter_dim=20, n_channels=1, hidden_layers=64, bidir=True,
        ...
        self.lstm_fwd = LSTM(BNLSTMCell, input_size=filter_dim * n_channels,
                             hidden_size=self.hidden_size, num_layers=self.num_layers,
                             max_length=30, batch_first=True)
        self.lstm_bwd = LSTM(BNLSTMCell, input_size=filter_dim * n_channels,
                             hidden_size=self.hidden_size, num_layers=self.num_layers,
                             max_length=30, batch_first=True, backward=True)
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x_fwd, _ = self.lstm_fwd(x)
        x_bwd, _ = self.lstm_bwd(x)
        x = torch.cat((x_bwd, x_fwd), 2)
        x = self.dropout(x)
